...
export class GameController {
  ...
  @Get(':id')
  async findById(@Param('id') id: string): Promise<GameResponseDto> {
  ...
  async recordThrow(
    ...
    const updatedGame = await this.gameService.recordThrow(id, recordThrowDto);
  ...
  async rollBackThrow(
    ...
    const rolledBackGame = await this.gameService.rollBackThrow(id);
    return plainToInstance(GameResponseDto, rolledBackGame.toObject());
  }
